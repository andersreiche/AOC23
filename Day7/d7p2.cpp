...
int getCardValue(char c) {
  std::string valueOrder = "J23456789TQKA";
...
int getHandValue(const std::string& str) {
...
int getHandType(const std::string& str) {
  bool joker = (str.find('J') != std::string::npos);

  // If there is a joker. substitute the joker with every card and return the highest hand.
  if (joker) {
    int highestHand = 0;
    std::string cards = "23456789TQKA";
    for (char replacementChar : cards) {
      std::string inputString = str;
        ...
        int handScore = getHandValue(inputString);
        if (handScore > highestHand) {
          highestHand = handScore;
        }
    }
    return highestHand;
  } else {
    return getHandValue(str);
  }
