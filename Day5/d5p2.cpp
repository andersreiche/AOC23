...
struct Seed_Entry {
  ...
  Seed_Entry(uint64_t start, uint64_t len) : startRange(start), length(len){}
};

std::vector<Seed_Entry> seeds;
...
uint64_t run() {
  std::vector<uint64_t> inputSeeds;
  ...
  uint64_t num = 0;
  while (iss >> num) {
      inputSeeds.push_back(num);
  }
  for (size_t i = 0; i < inputSeeds.size(); i += 2) {
      const auto& seed = inputSeeds[i];
      const auto& nextSeed = inputSeeds[i + 1];
      seeds.push_back(Seed_Entry(seed, nextSeed));
  }
  ...
  for (const auto& seed : seeds) {
    for (uint64_t i = seed.startRange; i < seed.startRange + seed.length; i++) {
      uint64_t location = getLocationFromSeed(i);
